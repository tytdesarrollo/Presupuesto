...
<?php
	...
	function agregarItemLista(){
		...
		var MesFact = document.getElementById("mesFact").value;
		var codigoF = document.getElementById("codFrz").value;
		var codigoE = document.getElementById("codESM").value;
		var pagoAut = document.getElementById("rdn").value;
		var antiAut = document.getElementById("amant").value;
	...
	function eliminarItemLista(posicionEliminar){
		...
		for(var i=0 ; i<arrayCodigosFuerza.length ; i++){
			if(i != posicionEliminar){
				...
				arrayTempMesFac.push(arrayMesFac);
				...
				arrayTempAutAntic.push(arrayAutAntic[i]);
			}
		}
	...
	function guardarListado(){
		...
		$.ajax({
			url:'<?php echo Url::toRoute(['site/guardarautorizacionpago']); ?>',
			method: "GET",
	...
	function botonAgregar(){		
		...
		if (pagoAut == "" || bandera == 0 || codidoD == "" || antiAut == "") {		
		console.log(bandera)	;
