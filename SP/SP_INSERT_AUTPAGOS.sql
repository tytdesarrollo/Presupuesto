USE [presama]
GO
/****** Object:  StoredProcedure [dbo].[SP_INSERT_AUTPAGOS]    Script Date: 12/10/2017 16:28:09 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[SP_INSERT_AUTPAGOS] 
(
	...
	@IN_AMORTI			varchar(max),
	@IN_MES_FACT		varchar(max)
)
AS
	SET NOCOUNT ON
	...
	DECLARE @Posicion9 int
	DECLARE @Posicion10 int
	...
	DECLARE @AMORTI varchar(200)
	DECLARE @MES_FACT varchar(200)
	----------------------------------------------
	...
	SET @IN_AMORTI = @IN_AMORTI + ','
	SET @IN_MES_FACT = @IN_MES_FACT + ','
	...
	WHILE patindex('%,%' , @IN_NUM_DOCU) <> 0
		BEGIN
			...
			SELECT @Posicion9 =  patindex('%,%' , @IN_AMORTI)
			SELECT @Posicion10 =  patindex('%,%' , @IN_MES_FACT)		
			...
			SELECT @AMORTI = left(@IN_AMORTI, @Posicion9 - 1)
			SELECT @MES_FACT = left(@IN_MES_FACT, @Posicion10 - 1)			
			...
			VALUES 
			(@MODA,@COD_FUERZA,'CONSORCIO SAMA','AUT_PAG',@VIG,@FEC_MOV,@NUM_DOCU,@V_MPRMOVLIN,@COD_ESM,
			@V_NOM_ESM,'','',@VAL_AUTO,'1',@AMORTI,@MES_FACT,@FEC_AUT) 			
			...
			SELECT @IN_AMORTI = stuff(@IN_AMORTI, 1, @Posicion9, '')
			SELECT @IN_MES_FACT = stuff(@IN_MES_FACT, 1, @Posicion10, '')			
