...
AS
	...
	SET @CONTROLLER = 
		...
		INNER JOIN AUDI_SATELITE_PRESUP B
			ON A.CODI_ESM = B.COD_SATELITE
	...
	ELSE	
		...
		SET @SELECTNULL = '';
		SET @TABTEMP = N'CREATE TABLE ##TablaReporte2s (COD_ESM VARCHAR(15), CODI_SAT VARCHAR(15), ';

			IF(ISNULL(OBJECT_ID('tempdb..##TablaReporte2s'), 0) <> 0)	
				BEGIN
					DROP TABLE ##TablaReporte2s;
				END
		...
		SET @TSQL =	
			...
			INNER JOIN AUDI_SATELITE_PRESUP C
				ON C.COD_SATELITE = A.CODI_ESM;';
		
		-- SE CREA LA TABLA TEMPORAL			
		EXEC(@TABTEMP);

		INSERT INTO ##TablaReporte2s
		EXEC(@TSQL);

		SELECT *
		FROM ##TablaReporte2s A